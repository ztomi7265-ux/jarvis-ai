...
def clasificar_accion_nota(mensaje, notas_existentes):
    ...
    if notas_existentes:
        lista_titulos = "\n".join(f"- id:{n['id']} | título: {n['titulo']}" for n in notas_existentes)
    else:
        lista_titulos = "(el usuario no tiene notas guardadas todavía)"
...
{{"accion": "crear" | "modificar" | "abrir" | "ninguna", "nota_id": "id exacto de la lista de arriba si accion es modificar o abrir y hay una coincidencia clara, si no null", "contenido": "para crear: el contenido a guardar. Para modificar: SOLO lo nuevo que hay que agregar o cambiar. Para abrir o ninguna: cadena vacía"}}

Reglas:
- "crear" es cuando pide guardar/anotar algo nuevo sin referirse a una nota que ya existe.
- "modificar" es cuando pide agregar, cambiar o actualizar una nota que ya existe (debe coincidir claramente con una de la lista).
