...
# ---------- MAPA (buscar lugares, gratis con OpenStreetMap) ----------

def buscar_lugar_nominatim(consulta, limite=5):
    try:
        resp = requests.get(
            "https://nominatim.openstreetmap.org/search",
            params={"q": consulta, "format": "json", "limit": limite, "addressdetails": 1},
            headers={"User-Agent": "JarvisAsistentePersonal/1.0 (uso personal)"},
            timeout=10,
        )
        resp.raise_for_status()
...
@app.route("/api/mapa/buscar", methods=["GET"])
def buscar_lugar():
    consulta = (request.args.get("q") or "").strip()
    if not consulta:
        return jsonify([])
    return jsonify(buscar_lugar_nominatim(consulta))
...
def chat():
    ...
    # Comando: "crea/empieza/abre un chat nuevo" -> le dice al frontend que
    # limpie la pantalla y arranque una conversación en blanco.
    if not imagen and re.match(r"(?i)^(?:crea|crear|empieza|empezar|inicia|iniciar|abre|abrir)\s+(?:un\s+)?(?:nuevo\s+)?chat\b", mensaje_usuario):
        return jsonify({"respuesta": "Listo, empecemos un chat nuevo. ¿En qué te ayudo?", "nuevo_chat": True})

    # Comando: "muéstrame/abre el mapa de X" o "dónde queda X" -> busca y abre el mapa ahí
    match_mapa = re.match(
        r"(?i)^(?:(?:abre|muéstrame|muestrame|enséñame|enseñame|busca)\s+(?:el\s+)?mapa\s+de\s+|d[oó]nde\s+queda\s+)(.+)",
        mensaje_usuario
    ) if not imagen else None
    if match_mapa:
        lugar = match_mapa.group(1).strip().rstrip("?").strip()
        resultados = buscar_lugar_nominatim(lugar, limite=1)
        if resultados:
            r = resultados[0]
            texto_resp = f"Ahí lo tienes: {r['nombre']}."
            return jsonify({"respuesta": texto_resp, "mapa_abrir": r})
        else:
            return jsonify({"respuesta": f"No encontré «{lugar}» en el mapa."})
